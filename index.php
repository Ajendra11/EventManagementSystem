...
// Fetch search results based on GET parameters (filters)
$results    = search_events($_GET);

// Get list of all event categories for dropdown
...
<div class="container section">
    ...
    <form class="panel filters" method="get" style="margin-bottom:1.5rem;">
        ...
        <!-- Search input (title, category, location) -->
        <div>
            <label>Search</label>
        ...
        <div>
            ...
            <select name="category">
                ...
                <?php foreach ($categories as $cat): ?>
                    <!-- Keep selected value after search -->
    ...
    <?php else: ?>
        ...
        <div class="event-grid section">
            ...
            <?php foreach ($results['items'] as $event): ?>
                ...
                <!-- Get event status (Open, Sold Out, etc.) -->
                <?php $status = event_status($event); ?>

                <article class="card event-card">

                    <!-- If event has a banner image -->
                    ...
                    <div class="content">
                        ...
                        <!-- Short description (trimmed to 90 characters) -->
                        <p class="muted">
                            <?= e(mb_strimwidth($event['description'], 0, 90, '…')) ?>
                        </p>
                        ...
                        <?php if ((float)$event['price'] > 0): ?>
                            <p class="price-tag">
                                Rs. <?= e(number_format((float)$event['price'], 0)) ?>
                            </p>
                        <?php else: ?>
                        ...
                        <a class="btn btn-outline btn-block" 
                           href="<?= APP_URL ?>/events/show.php?id=<?= (int)$event['id'] ?>">
                           View details
                        </a>
                    </div>
                </article>
